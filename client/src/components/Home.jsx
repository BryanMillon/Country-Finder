...
export default function Home(){
  ...
  let lastIdx = currentPage===1 ? currentPage * countriesPerPage:currentPage * countriesPerPage-1 // en la primera página, lastIdx = 1 * 9 = 9
      ...
      !allCountries.length? <Loader/> :
        <div className={styles.background}>
          ...
          <div className={styles.nav}>
            <button className={styles.reload} onClick={e=>{handleClick(e)}}>RELOAD COUNTRIES AND CLEAN FILTERS</button> 
            <h1 className={styles.title}>C🌎UNTRY F🔍NDER</h1>
          <Link  className={styles.link} to= '/activities'> Create Activity</Link>
          </div>
...
{/* <div className={styles.filter}> */}
            <select value={continentF} onChange={e=>{handleFilterContinent(e)}}  className={styles.filter}>
            <option hidden value='Continents'>Continents</option>
          ...
          <select value={az} onChange={e=>{handleFilterName(e)}}  className={styles.filter}>
            <option hidden value='Alphabetical Order'>Alphabetical Order</option>
            <option  value="asc">A-Z</option>
            <option value="desc">Z-A</option>
          </select>

          <select value={populationF} onChange={e=> handleFilterPopulation(e)}  className={styles.filter}>
            <option hidden value='Population' >Population</option>
            <option value="asc">📈</option>
            <option value="desc">📉</option>
          </select>

          <select value={activitiesF} onChange={e=> handleFilterActivity(e) }  className={styles.filter}>
            <option hidden value='All' >Activities</option>
            {activities?.map((act) => (
          ...
          {currentCountries?.map(el => {
                return(
                  <div  key={el.id} className={styles.containerCard}>
                      <Link className={styles.link} to={`/countries/${el.id}`} key={el.id}>
